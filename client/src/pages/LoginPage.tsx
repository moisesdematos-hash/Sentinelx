...
export const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === 'LOGIN') {
        // First try local API, then fallback to Supabase Auth
        try {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', background: 'var(--bg-primary)' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', margin: '4px 0' }}>
          ...
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>ou com e-mail corporativo</span>
          <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.1)' }} />
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          {authMode === 'REGISTER' && (
            <>
              <div>
                <label style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>Nome Completo</label>
              ...
              <div>
                <label style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>Nome da Organização</label>
                <input
                  type="text"
                  required
          ...
          <div>
            <label style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>E-mail Corporativo</label>
            <div style={{ position: 'relative' }}>
          ...
          <div>
            <label style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>Senha de Acesso</label>
            <div style={{ position: 'relative' }}>
          ...
          >
            {isSubmitting ? 'Autenticando...' : authMode === 'LOGIN' ? 'Acessar Console SentinelX' : 'Criar Conta no Supabase'}
            <ArrowRight size={16} />
          </button>
        </form>
        ...
        >
          <Sparkles size={16} /> Entrar em Modo Convidado (Demo Instantânea)
        </button>
      </div>
    </div>
  );
